...
function whatRepo() {
  ...
  elif ! ( cd "$repo" && git rev-parse --git-dir &> /dev/null ); then
  ...
  if [ -n "$gitLoc" ]; then
    ...
    if [ $retval -eq 0 ]; then
      ...
      if [ -z "$changes" ]; then
        ...
        echo -e "\n$repo is not up to date, updating from GitHub:"
        git pull
        retval=$?
        if [ $retval -ne 0 ]; then
          ...
          echo -e "'cd $repo; sudo git stash; sudo git pull'"
    ...
    echo -e "\nNo valid repo passed to function ($repo)."
  fi
}

updateRepo "/home/pi/brewpi-tools-rmx" || echo -e "\nError updating: /home/pi/brewpi-tools-rmx"
updateRepo "/home/brewpi" || echo -e "\nError updating: /home/brewpi"
updateRepo "/var/www/html" || echo -e "\nError updating: /var/www/html"
