...
############
### Make sure user is running with sudo
############
...
warn() {
  ...
  echo -e "Script execution not completed.\n"
}
...
if [ -z "$scriptpath" ]; then
    ...
    sed -i '1iscriptpath="/home/brewpi"' "$cronfile"
fi

if [ -z "$stdoutpath" ]; then
    stdoutpath="/home/brewpi/logs/stdout.txt"
    echo -e "No previous setting for stdoutpath found, using default:\n$stdoutpath\n"
    sed -i '1istdoutpath="/home/brewpi/logs/stdout.txt"' "$cronfile"
fi

if [ -z "$stderrpath" ]; then
    stderrpath="/home/brewpi/logs/stdout.txt"
    echo -e "No previous setting for stdoutpath found, using default:\n$stderrpath\n"
    sed -i '1istderrpath="/home/brewpi/logs/stderr.txt"' "$cronfile"
fi
  ...
  if [[ ! -z "$wlan" ]]; then
    ...
    case "$yn" in
      y | Y | yes | YES| Yes )
          ...
          sed -i '/entries=.*/ s/"$/ wifichecker"/' "$cronfile"
          checkEntry wifichecker "$wificheckcron"
          bash "$scriptpath"/utils/wifiChecker.sh --checkinterfaces
          ;;
      * )
          # update entries="..." to entries="... ~wifichecker"
          sed -i '/entries=.*/ s/"$/ ~wifichecker"/' "$cronfile"
