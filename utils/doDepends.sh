...
# nginx packages to be uninstalled via apt
...
############
### Remove brewpi user/group config
############
...
echo
read -p "Would you like to remove apt and pip packages? [y/N] :" yn < /dev/tty
if [[ "$yn" == [Yy]* ]]; then
  # Remove apt and pip packages
...
# Cleanup
echo -e "Cleaning up local packages."
sudo apt clean -y
