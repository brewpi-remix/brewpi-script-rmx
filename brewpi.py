...
serialCheckInterval = 0.5  # Blocking socket functions repeat time in seconds
...
def getGit():
    ...
    try:
        ...
        # We were unable to access git
        logError('Unable to access Git information. Check permissions.')
        return False
...
def loop():  # Main program loop
    ...
    try:  # Main loop
        while run:
            ...
            try:  # Process BEERSOCKET messages (to/from PHP)
                handleBeerSocket()
            ...
            except ConnectionError as e:  # Connection freaked out, report it and exit
                type, value, traceback = sys.exc_info()
                fname = os.path.split(traceback.tb_frame.f_code.co_filename)[1]
...
def handleBeerSocket():  # Handle BEERSOCKET communications
    global config
    global hwVersion
    global lcdText
    global statusType
...
def handleSerialConnection():  # Handle controller connection
    global config
    global hwVersion
    global lcdText
    global cs
    ...
    # Controller not responding
        ...
        logError(
            "Controller is not responding to new data requests.")
...
def main():
    ...
    if not getGit():  # Retrieve git (version) information
        sys.exit(0)
